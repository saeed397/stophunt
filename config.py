...
IMPORTANT — Rule #2/#3 compliance (no invented API data, only documented sources):
  - Data provider details (Yahoo Finance via `yfinance`, CoinGecko OHLC) are
    documented in full in data/providers.py's module docstring, including the
    exact endpoints/params and their documented limits. This file only holds
    the small lookup tables those providers need.
  - CryptoCompare has been REMOVED as of this revision: its free tier is a
    7-day trial and Iran cannot purchase a paid plan to continue past it —
    flagged directly by the user. See data/providers.py for the replacement
    Multi-Provider Strategy (Yahoo Finance primary, CoinGecko fallback).
...
# Yahoo Finance documented intraday lookback limits, in days, keyed by the
# native yfinance interval string. Source: yfinance / Yahoo chart API docs
# (github.com/ranaroussi/yfinance). Daily/weekly/monthly have no practical
# limit here, so they're simply absent from this dict (treated as unlimited).
...
class BacktestCalibrationDefaults:
    ...
    """
    min_lookback_candles: int = 300          # minimum TOTAL history required before any detection runs
    rolling_calibration_window: int = 300    # how many of the MOST RECENT candles feed the statistical
                                              # thresholds (tolerance, penetration depth, wick/body ratio).
                                              # Kept separate from min_lookback_candles on purpose: an asset
                                              # can have 2000 candles of total history (so swing/liquidity
                                              # detection sees the full structural picture) while its SL/TP
                                              # sizing is still calibrated only to its CURRENT volatility
                                              # regime, not diluted by a stale quiet period from months ago.
