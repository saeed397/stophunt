...
class CryptoCompareProvider:
    ...
    def __init__(self, api_key: Optional[str] = None, session: Optional[requests.Session] = None):
        # اگر کلید مستقیم داده نشد، از طریق استریم‌لیت تلاش کن
        if api_key is None:
            try:
                api_key = st.secrets["crypto_api"]["key"]
            except Exception:
                api_key = None
        ...
        # تنظیم هدر احراز هویت طبق مستندات CryptoCompare
        if self.api_key:
    ...
    def get_ohlcv(self, symbol: str, quote: str, timeframe: str,
        ...
        # اگر از طریق هدر داده شده باشد، اینجا پارامتر اضافه نمی‌کنیم تا تداخل ایجاد نشود
        # اما برای اطمینان در برخی نسخه‌ها، ارسال مجدد پارامتر api_key هم ضرری ندارد
        if self.api_key:
