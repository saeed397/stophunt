...
def validate_candles(candles: List[Candle], asset: str, timeframe: str,
    ...
    """
    `actual_interval_seconds`: the REAL bar duration reported by the data
    provider (see data.providers.OHLCVBatch). Gap detection must use this,
    not the nominal timeframe label — a CoinGecko fallback batch may actually
    be 4-hour bars even though the user asked for '1d', and treating every
    gap-vs-nominal as a real data gap would misreport perfectly good data as
    broken. If not provided, falls back to the nominal timeframe's seconds
    (used by tests/offline paths that don't go through a live provider).
    """
    if not candles:
