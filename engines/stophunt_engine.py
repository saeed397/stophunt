...
from data.providers import Candle, MultiProviderOHLC
...
def run_signal_engine(asset: str, coingecko_id: str, quote: str, timeframe: str,
    ...
    if profile.regime_shift_flag:
        notes.append(
            f"⚠️ تغییر رژیم نوسان شناسایی شد: ATR اخیر این دارایی نسبت به دوره قبل از آن "
            f"{profile.regime_shift_ratio}x تغییر کرده است. آستانه‌ها با داده تازه کالیبره شده‌اند، "
            f"اما در تصمیم‌گیری این نکته را در نظر بگیرید."
        )
    ...
    return EngineResult(
        ...
        setup_scores=setup_scores, notes=notes, data_source=batch.source,
    )
